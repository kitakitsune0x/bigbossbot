...
import { getBigBossFeed, getBigBossSnapshot, searchBigBossIntel } from '@/lib/mcp/intel';
...
test('getBigBossFeed normalizes nested feed payloads', async () => {
  globalThis.fetch = (async (input: RequestInfo | URL) => {
    ...
    assert.equal(url.pathname, '/api/alerts');
    assert.equal(url.searchParams.get('theater'), 'ukraine');
